...
WITH comp_wallet_list AS (
  ...
  comp_contract_address_list_ethereum AS (
    SELECT
      0xc3d688B66703497DAA19211EEdff47f25384cdc3 AS contract_address, --cUSDCv3 on mainnet
      6 AS underlying_decimals,
      'USDC' AS token,
      0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48 AS token_address -- USDC on mainnet 
  ),
  comp_rates_raw_ethereum AS (
    ...
    FROM
      ...
      LEFT JOIN compound_v3_ethereum.Comet_evt_Transfer b ON a.evt_tx_hash = b.evt_tx_hash
    ...
    SELECT
      ...
      (b.amount * 1e0) AS delta_ctok
    FROM
      compound_v3_ethereum.Comet_evt_Supply a
      LEFT JOIN compound_v3_ethereum.Comet_evt_Transfer b ON a.evt_tx_hash = b.evt_tx_hash
      JOIN comp_contract_address_list_ethereum c ON a.contract_address = c.contract_address
   ...
   aprrate_ethereum AS (
    ...
    from
    compound_v3_ethereum.Comet_call_getSupplyRate as supply_rates
  ...
  from (
    ...
    SELECT
      ...
      1 AS line,
      CASE
        WHEN token = 'USDC' THEN 112001 /* increase in money market assets COMP USDC */
      END AS account,
      'P&L' AS ledger,
      ts,
    ...
    SELECT
      ...
      2 AS line,
      CASE
        WHEN token = 'USDC' THEN 32321001 /* increase in investment P&L COMP USDC */
      END AS account,
      'P&L' AS ledger,
      ts,
      interest_accrual / POWER(10, underlying_decimals) AS amount,
